...
// Backend base URL for displaying payment proofs
...
export default function AdminDeposits() {
  ...
  const STATUS_TABS = [
    { val: 'pending',  label: 'Pending',  color: 'var(--accent)' },
    { val: 'approved', label: 'Approved', color: 'var(--green)' },
    { val: 'rejected', label: 'Rejected', color: 'var(--red)' },
  ]

  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: '1.5rem' }}>
      <div>
        <h1 style={{ fontFamily: '"Poppins", sans-serif', fontSize: '1.8rem', fontWeight: 700, color: 'var(--text-primary)', marginBottom: 4 }}>Deposits</h1>
        <p style={{ color: 'var(--text-secondary)', fontSize: 14 }}>Review and manage deposit requests</p>
      </div>
        ...
        <table className="data-table">
          ...
          <tbody>
            {loading ? (
              ...
              <tr><td colSpan={6} style={{ textAlign: 'center', padding: '3rem', color: 'var(--text-muted)' }}>No deposits found</td></tr>
            ) : deposits.map(d => (
              <tr key={d._id}>
                ...
                <td style={{ textAlign: 'center', color: 'var(--text-muted)', fontSize: 12 }}>{new Date(d.createdAt).toLocaleDateString('en-US')}</td>
        ...
        <div style={{ position: 'fixed', inset: 0, zIndex: 60, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '1rem' }}>
          ...
          <div className="card" style={{ position: 'relative', width: '100%', maxWidth: 380, zIndex: 1 }}>
            ...
            <div>
              ...
              <textarea value={note} onChange={e => setNote(e.target.value)} className="input" style={{ height: 80, resize: 'none' }} placeholder="Message to the user…" />
            </div>
